...
export const PCCertDialog: React.FC<PCCertDialogProps> = ({ onClose }) => {
  ...
  return (
    <Dialog
      isOpen
      onClose={onClose}
      title="Root Certificate Authority (CA) Manager"
      subtitle="Install and trust Root CA for OS browsers, CLI tools, and Java runtimes."
      ...
      <div className="flex flex-col gap-4 text-xs">
        ...
        >
          ...
          >
            ...
            <span>Java Keystore ({javaInstalls.length})</span>
          </button>
        </div>
        ...
        {activeTab === 'system' ? (
          <div className="flex flex-col gap-3.5">
            ...
            >
              ...
              <div
                className="flex items-center justify-between py-1 border-t pt-2.5"
          ...
          <div className="flex flex-col gap-3 min-h-0 flex-1">
            <div className="flex items-center justify-between">
              <span className="text-[11px]" style={{ color: 'var(--color-text-muted)' }}>
                Inject Root CA into Java JDK / JRE keystores (<code>cacerts</code>) for Maven, Gradle, Spring Boot, and OkHttp.
              </span>
            ...
            <div
              className="flex-1 max-h-[280px] overflow-y-auto border rounded-2xl p-2 space-y-2"
              style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface-raised)' }}
            >
              {javaInstalls.length === 0 ? (
                <div className="py-12 text-center text-xs" style={{ color: 'var(--color-text-subtle)' }}>
                  ...
                  <p className="text-[11px] mt-1">Click "Browse JDK..." to select your custom JDK directory</p>
                </div>
              ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {inst.isInstalled ? (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-400 flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" />
                            <span>Trusted in cacerts</span>
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-neutral-500/15 text-neutral-400">
                            Not Installed
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] font-mono truncate mt-1" style={{ color: 'var(--color-text-subtle)' }} title={inst.path}>
                        {inst.path}
                      </p>
                    </div>
